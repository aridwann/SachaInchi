                ...
                <table class="min-w-full text-sm text-[#A3C299]">
                    <thead class="bg-[#172112]">
                        <tr>
                            ...
                            <th class="text-left p-3">Hapus Produk</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($cart as $productId => $item)
                            ...
                            <tr id="row-{{ $productId }}" class="border-t border-[#A3C299]">
                                <td class="p-3 flex items-center gap-3">
                                    <input type="checkbox" class="w-4 h-4 cursor-pointer checkbox-item" data-id="{{ $productId }}" />
                                ...
                                <td class="p-3">
                                    <div class="flex items-center gap-2">
                                        <button onclick="updateQty('{{ $productId }}', -1)" class="size-6 text-xl font-bold">-</button>
                                ...
                                <td class="p-3 flex justify-center items-center">
                                    <button onclick="confirmRemove('{{ $productId }}')" class="text-red-500 hover:text-red-700 text-xl">🗑️</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            fetch(`/cart/update/${id}/${qty}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('total-' + id).textContent = 'Rp ' + formatRupiah(data.cart[id].price * data.cart[id].quantity);
